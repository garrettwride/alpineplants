...
for (pa_strat in pa_strategies) {
  myBiomodData <- BIOMOD_FormatingData(
    ...
    PA.dist.min = disk_min,
    PA.dist.max = disk_max#,
  ...
  for (algo in algorithms) {
    plot_title <- paste0(sp$Species[1],
                         "\nPA Strategy: ", pa_strat, " (", algo, ")")
    plot(get_model_layer(r, algo), main = plot_title)
  }
}

#2
sp <- species_df %>% 
  filter(Species == "Campanula rotundifolia")
respXY <- sp$Occurrences[[1]][, c("decimalLongitude", "decimalLatitude")]
resp   <- rep(1, nrow(respXY))
...
#3
sp <- species_df %>% 
  filter(Species == "Delphinium occidentale")
respXY <- sp$Occurrences[[1]][, c("decimalLongitude", "decimalLatitude")]
resp   <- rep(1, nrow(respXY))
